...
def app():
    ...
    Tray1_databasedPath = 'Golding_Tray1_Database_update.xlsx'     
    Tray1_sensorResults = process_sensor_data(Tray1_databasedPath)

    Tray2_databasedPath = 'Golding_Tray2_Database_update.xlsx'      
    Tray2_sensorResults = process_sensor_data(Tray2_databasedPath)
    ...
    ###################      Start of App     ###############################
    st.subheader("Golding Wear Sensor Trial - Komatsu 830E Hercules Tray", divider = 'rainbow')
    ...
    Tray1, Tray2 = st.tabs(["Hercules Tray 1", "Hercules Tray 2"])
    with Tray1:
        st.image("tray1.png", caption = "Tray 1 Sensor Install Locations", use_container_width = True)
    with Tray2:
        st.image("tray2.png", caption = "Tray 2 Sensor Install Locations", use_container_width = True)
    ...
    Tray1_sensor, Tray2_sensor = st.tabs(["Hercules Tray 1", "Hercules Tray 2"])
    with Tray1_sensor:
        for row in Tray1_sensorResults.itertuples():
        ...
        fig1 = plot_sensor_data(Tray1_databasedPath)
        st.plotly_chart(fig1, use_container_width=True)

        # data download function
        downloadData(Tray1_databasedPath)


    with Tray2_sensor:
        for row in Tray2_sensorResults.itertuples():
            # 检查 totalLength 是否非空（非 NaN）
            if pd.notna(row.totalLength):
                st.caption("Latest Reading at: " + str(row.latestTime))
                st.metric(label = ":material/Sensors: " +  row.sensorName + " Sensor Reading", value = str(row.actualLength) + "mm", delta = row.actualLength - row.totalLength, border=True)
        ...
        # Specify the file path
        fig2 = plot_sensor_data(Tray2_databasedPath)
        st.plotly_chart(fig2, use_container_width=True)
    
        # data download function
        downloadData(Tray2_databasedPath)
